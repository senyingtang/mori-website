  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-white/10 bg-white/[0.06] px-6 py-8 backdrop-blur-md">
        <h1 className="text-2xl font-bold text-white">聯絡訊息</h1>
        <p className="mt-2 text-sm text-white/55">
          檢視與標記 <code className="rounded bg-black/30 px-1">contact_submissions</code>
          ，目前僅寫入 DB，不寄信、不串報名系統。
        </p>
        {error ? (
          <p className="mt-3 text-sm text-red-200/90">讀取失敗：{error.message}</p>
        ) : null}
      </div>
